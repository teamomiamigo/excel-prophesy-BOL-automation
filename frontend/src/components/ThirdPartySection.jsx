...
export default function ThirdPartySection({ bols, approvingId, unmarkingThirdPartyId, onApprove, onUnmark }) {
    ...
    <section style={{ marginBottom: 28 }}>
      <details>
        <summary style={{
          ...
          ▸ Third Party — {bols.length} record{bols.length !== 1 ? 's' : ''} (customer pays freight directly — excluded from Prophecy export)
        </summary>
        <div style={{ overflowX: 'auto', borderRadius: '0 0 8px 8px', border: '1px solid #fed7aa', borderTop: 'none' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', background: '#fff' }}>
            ...
            <tbody>
              {bols.map(bol => {
                const isApproving = approvingId === bol.id;
                  ...
                  <tr key={bol.id} style={{ background: '#fffaf5' }}>
                    ...
                    <td style={{ ...TD, textAlign: 'center' }}>
                      <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
                        <button
                          onClick={() => onApprove(bol.id)}
                          disabled={isApproving}
                          title="Approve this third-party record"
                          style={{

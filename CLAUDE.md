...
**`access_prog` calculation (last corrected 2026-07-14 — supersedes the 2026-07-06/#40 version, which this section previously described stale):** Computed in `_apply_access_prog_calc()`, shared by `_process_invoice_csv()` (every invoice upload, including Wolf/311 stub creation) and the `POST /api/admin/recompute-access-prog` backfill. Weight/pallets/pieces must always be **ours**, never ALG's — but the tariff/zone rate structure is legitimately **ALG's own pricing**, so using it is correct, not a violation of independence:
1. Get our own per-pallet `(zone, weight)`: `get_pallet_data_for_manifests()` for Corp/Technique trips, `get_prophecy_pallet_data()` for Wolf/311 (Prophecy-matched) loads. Prefer the pallet's actual postal `destination_zip` over the SCF zone code embedded in `destination_id` (e.g. "SCF350") — ALG bills the real zip3, not the SCF code, so joining on `destination_id` misses almost every zone (confirmed live on invoice Z558429: 0/33 zones matched before this was fixed). **If our own data comes back empty (manifest/BOL not found, not yet synced), `access_prog` is left `null`** — no independent data means no independent estimate; there is no fallback to ALG's weight.
2. Per pallet (zone, our weight), resolve the rate in priority order: (a) ALG's own invoiced rate for that zone — read directly from the CSV's `Rate` column (confirmed populated and accurate on 126 real historical invoices; do **not** derive it as `Billed$/GrossWt`, which silently bakes in ALG's per-shipment minimum-freight charge as if it were a flat rate whenever a shipment was small enough to hit that floor — e.g. a $70 minimum on a 216 lb parcel implies a fake ~$32/cwt "rate" that overstates cost badly when applied to a differently-sized pallet on our side); exact zip3 match first, else nearest invoice zone within `_ALG_ZONE_TOLERANCE` (±5); (b) if the invoice didn't bill that zone at all, our internal `tariff_rates` card as a fallback — sets `tariff_zone_approximate = true` when this happens. **Either way, apply the zone's own minimum freight charge** (`tariff_rates.minimum_freight`, surfaced via `get_tariff_rate()`'s `minimum_freight` return key) as a floor on the computed cost — our own card's minimum matches ALG's actual charged minimum exactly, zone for zone, so this must be applied on the ALG-rate path too, not just the internal-card fallback path (which already had this floor built in).
3. If per-zone rating covers less than `_RATE_COVERAGE_THRESHOLD` (80%) of the load's weight, discard the per-zone sum and price the whole load at the invoice's own blended $/cwt (`alg_blended_rate` = total freight billed ÷ total billed weight) instead, or leave `access_prog` null if no blended rate is available either — a partial per-zone match previously produced 800–1300% readings before this threshold existed.
4. FSC comes from the invoice's own "Fuel Surcharge" footer row (`alg_fsc_pct`/`alg_fsc_cost` columns), not EIA, per above.

...
| POST | `/api/admin/recompute-invoice-senders` | One-time backfill re-parsing stale raw `invoice_email_sender` folder-name strings into clean display strings + `invoice_sent_at`, for records created before the parser correctly handled their shape. Idempotent; live mode only |
| GET | `/api/admin/rate-table-counts` | Read-only row counts for the three static rate tables (`tariff_rates`/`alg_tariff_rates`/`fuel_surcharge_rates`). Added 2026-07-22 after discovering live Aurora's `tariff_rates`/`fuel_surcharge_rates` were completely empty and `alg_tariff_rates` only ~58% seeded — nothing in the deploy pipeline seeds data, and Aurora is VPC-private with no Data API, so there was no direct way to run `seed_rates.py`'s CLI against it. Wrapped into the `/cleanout` skill's report so a future gap is visible immediately |
| POST | `/api/admin/seed-rate-tables` | (Re-)seed the three static rate tables from S3 (`s3://{INVOICE_S3_BUCKET}/rate-seed/tariff_rates.csv` / `fsc_matrix.xlsx` / `alg_tariff_rates.csv`) — the fix for the gap above. Reuses `backend/seed_rates.py`'s own loader functions unchanged (same DELETE-then-bulk-insert replace semantics), just sourcing files from S3 instead of local disk paths the Lambda can't reach. Safe to re-run; returns before/after counts |
...
| POST | `/api/invoices/fix-sender` | Manual correction for a sender batch whose folder name failed to parse at upload/poll-folder time — re-parses a user-supplied corrected folder name and moves every record sharing the original raw `invoice_email_sender` onto it. Powers `InvoiceSenderFixBanner.jsx` |
...
**This was never done against live Aurora** (found 2026-07-22): `tariff_rates` and `fuel_surcharge_rates` were completely empty (0 rows) and `alg_tariff_rates` was only ~58% seeded (305/528 rows, all accumulated from the self-learning `reconcile_alg_tariff_rates()` mechanism, not the original CSV import) — confirmed via CloudWatch `[ZONE GAP]` warnings recurring on fresh live invoice uploads for zip3s local's copy of `tariff_rates` had no trouble with. Root cause: Aurora is VPC-private with no Data API, so there was never a direct way to run the CLI above against it, and nothing in the deploy pipeline (`deploy.ps1`) seeds data — only code/infra. Fixed by adding `GET /api/admin/rate-table-counts` (diagnostic) and `POST /api/admin/seed-rate-tables` (the fix — reads the same three source files from `s3://{INVOICE_S3_BUCKET}/rate-seed/` and calls this same CLI's loader functions) — see the API Routes table above. Re-run `seed-rate-tables` any time `rate-table-counts` looks short (also checked by the `/cleanout` skill now).
...
- **AWP-SQL-PROD connect timeout** (`data_layer.py`'s `_get_connection()`): 8s, not pyodbc's default 30s (fixed 2026-07-21) — a 30s connect attempt is longer than the Lambda function's own 29s hard timeout (`lambda.tf`), so any slow/unreachable connection during the per-invoice wide-fallback search (any unmatched invoice on upload/poll-folder/poll-email) guaranteed an ungraceful Lambda kill (bare HTTP 500) instead of a fast, catchable failure. **Query timeout** (same function, `_get_connection(query_timeout=...)`, added 2026-07-21, scoped 2026-07-22): pyodbc's `Connection.timeout` bounds every `cursor.execute()` on a connection, separate from the connect-phase timeout above — added after a real invoice upload (Z557856) confirmed via CloudWatch that a query which connected fine could still hang past Lambda's 29s wall with zero traceback once inside `_wide_fallback_technique_search()`. The first version set this as a blanket `conn.timeout = 15` inside `_get_connection()` itself, which broke the main Technique pull (`get_technique_data()` in `pull_technique_data()`) the first time it ran afterward — that query's own live latency is ~15–23s (see the provisioned-concurrency note above), so a flat 15s cap killed it with `HYT00 Query timeout expired` instead of letting it finish. Fixed by making `query_timeout` an opt-in parameter on `_get_connection()`/`get_technique_data()`/`get_manifest_weights()`, left unset (pyodbc default: no timeout) everywhere except the two live calls inside `_wide_fallback_technique_search()`. `_wide_fallback_technique_search()` also catches any live-query failure (timeout or otherwise) and degrades to a normal `invoice_only` stub instead of propagating. Residual risk: a request chaining two live calls back-to-back (the ambiguous-trip scoring path) can still approach 29s even with this in place — a real fix needs a per-request deadline budget, not yet built.
- **`_call_with_timeout()` (`main.py`) runs its wrapped call in a forked child process, not a thread (fixed 2026-07-30, three rounds — see commit history for the two intermediate attempts that didn't hold up live)**. Root cause of the underlying outage: `retry-match`'s call to `_wide_fallback_technique_search()` had `query_timeout=None` (the 2026-07-22 refactor's own choice, on the theory that this route has the full request budget to itself), meaning pyodbc's own query cutoff was disabled and the *only* backstop was this wrapper's soft deadline (`_WIDE_FALLBACK_DEADLINE=25`) implemented via `ThreadPoolExecutor`/`future.result(timeout=...)`. Confirmed live (job 111427/Z559268) that a thread-based wrapper cannot reliably enforce this: pyodbc/the ODBC driver doesn't release the GIL while blocked on the network, so the waiting thread can't wake up on schedule either — the whole Lambda invocation just ran to its own hard 29s kill instead, a raw uncatchable 500 instead of the intended graceful `{"timed_out": true}`. A `ProcessPoolExecutor` fixed *some* cases (proven live) but not this one — declining to wait on a stuck worker isn't the same as actually stopping it. The working fix: `multiprocessing.get_context("fork")` + `Process`/`Pipe` (not `Queue` — `Queue`'s internal `Lock` needs a POSIX semaphore via `sem_open()` under `/dev/shm`, which Lambda's execution environment doesn't reliably provide; confirmed live via `[Errno 2] No such file or directory` immediately on `Queue()` creation, which this function's own except-and-report-back design silently mislabeled as a timeout instead of a crash — always get the actual traceback, not just `str(exc)`, before trusting a "confirmed" fix). On timeout, the parent now explicitly `terminate()`s (then `kill()`s if needed) the stuck child, forcibly closing its socket to AWP-SQL-PROD, restoring `query_timeout` as a real, working backstop underneath the wrapper's own deadline.
- **`query_timeout` tuned same day, 15 → 22 → 24 → 22 (settled)**: once the fork+Pipe mechanism was proven reliable, 15s (borrowed from an unrelated call site) turned out to be needlessly conservative — confirmed live that the 90-day scan routinely needs more than 15s (multiple job suffixes all hit a clean `HYT00 Query timeout expired` at exactly 15s, never once completing). Pushing to 24s (with `_WIDE_FALLBACK_DEADLINE` also raised 25 → 27 alongside it) regressed on the very next live test to a raw Lambda hard-timeout crash — the query's real execution time varies run to run, and 27s left too little margin under Lambda's 29s ceiling for `terminate()`/`join()` cleanup. Settled on **22s query_timeout / 25s outer deadline**: reliable graceful degradation beats squeezing a few more seconds of query time at the cost of occasional raw crashes.
- **Known residual risk, accepted 2026-07-30**: even at the settled 22s/25s values, the fork+Pipe backstop has been observed to occasionally miss its own deadline — one live request logged *nothing at all* for ~28s before Lambda's hard kill, not even `_call_with_timeout()`'s own first line of code (confirmed via temporary per-step timestamped logging, since removed), with only trivial non-blocking Python between the prior DB fetch and that point. This rules out anything diagnosable in application code — it looks like an AWS Lambda infrastructure-level pause of the whole execution environment during an active invocation, not a bug this function can detect or work around. Accepted as a rare residual risk rather than continuing to chase it blind; the frontend's own auto-retry (up to 3 attempts per stub, `AUTO_RETRY_MAX_ATTEMPTS` in `App.jsx`) absorbs most occurrences. Re-verified live end to end after settling on 22s/25s: previously-stuck records reliably return clean `HTTP 200 {"timed_out": true}` in ~15-24s in the large majority of attempts.
- **Provisioned concurrency + alias updates during a version transition can look like a stuck/broken deploy — it isn't.** While a `PutProvisionedConcurrencyConfig` transition is `Status: IN_PROGRESS` (check via `aws lambda get-provisioned-concurrency-config --qualifier live`), AWS automatically adds a temporary `RoutingConfig.AdditionalVersionWeights` entry on the `live` alias pointing at the *previous* version — Terraform doesn't declare this, so `terraform plan`/`apply` will keep showing an unexpected `routing_config { - additional_version_weights }` diff, and trying to force it away with `update-alias` while the transition is still `IN_PROGRESS` fails outright (`InvalidParameterValueException: Alias with weights can not be used with Provisioned Concurrency`) or gets silently re-added. Just wait for the PC status to reach a terminal state (`READY` or `FAILED`, took ~50s–150s in practice) — AWS clears the routing weight on its own once it settles, and `terraform apply` then goes through cleanly.
...
- **`RDS_MASTER_SECRET_ARN` wiring is now live (fixed 2026-07-30)**: `backend/config.py` reads `DATABASE_URL` directly from Aurora's own auto-rotated secret at every cold start (via `RDS_MASTER_SECRET_ARN`/`DB_HOST`/`DB_PORT`/`DB_NAME` env vars on `aws_lambda_function.app` in `lambda.tf`), instead of the manually-synced copy in `sg360-bol-live-credentials`. The `iam:PutRolePolicy` block this previously needed had actually already been resolved by the time this was rechecked (`aws iam get-role-policy --role-name sg360-bol-lambda-exec --policy-name sg360-bol-lambda-secrets-access` already included the RDS secret ARN) — don't assume a past "blocked, needs IAM grant" note is still accurate without a 30-second check first. This eliminates the manual-resync outage class entirely going forward. A planned CloudWatch-alarm-on-Lambda-Errors → SNS email alert is still not built (needs `sns:CreateTopic`/`cloudwatch:PutMetricAlarm`, same account, permission status unchecked).

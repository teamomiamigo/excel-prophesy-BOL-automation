...
| POST | `/api/admin/pull` | Pull Technique manifests from AWP-SQL-PROD (disabled in mock mode). Also re-matches any stuck `invoice_only` stubs: first against already-committed DB records, then (if any remain, live query only when needed) a 90-day wide-fallback Technique search (widened from 21 days 2026-07-17 — an ALG invoice batch can lag well over 3 weeks behind despatch). Both paths now call `_finish_resolving_stub()` — previously they silently dropped `invoice_email_sender`/`invoice_sent_at` and never computed `access_prog`/`cost_pct`, unlike the main upload-time match |

...
| POST | `/api/admin/pull` | Pull Technique manifests from AWP-SQL-PROD (disabled in mock mode). Also re-matches any stuck `invoice_only` stubs: first against already-committed DB records, then (if any remain, live query only when needed) a 40-day wide-fallback Technique search (widened from 21 to 90 days 2026-07-17 — an ALG invoice batch can lag well over 3 weeks behind despatch — then narrowed to 40 days 2026-07-20: stacked onto the main 20-day pull in the same request, the 90-day version reliably pushed this endpoint past API Gateway's hard 30s integration timeout whenever any `invoice_only` stub existed, confirmed live post-deploy; the per-record Retry button below isn't stacked with anything else and safely keeps the full 90-day window). Both paths now call `_finish_resolving_stub()` — previously they silently dropped `invoice_email_sender`/`invoice_sent_at` and never computed `access_prog`/`cost_pct`, unlike the main upload-time match |
...
**`days_back` is 20** in `pull_technique_data()` — re-pull refreshes technique-side fields (weight/pallets/pcs, BOL status) but preserves invoice matches, approvals, flags, and notes on existing records. Safe to re-run any time. Its own stub-rematch wide-fallback pass uses a separate, wider `days_back=40` (see `/api/admin/pull` route entry above — narrowed from 90 on 2026-07-20 to stay under API Gateway's 30s timeout) — only the main manifest-discovery window is 20.
